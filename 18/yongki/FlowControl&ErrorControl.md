...
- [TCP 흐름제어 및 오류제어](#tcp-흐름제어-및-오류제어)
  - [TCP 흐름 제어](#tcp-흐름-제어)
  - [TCP 구조](#tcp-구조)
    - [이전 논제 [`ACK 번호는 어떻게 계산되는가`]](#이전-논제-ack-번호는-어떻게-계산되는가)
    - [이전 논제 [`데이터 송·수신의 3-way handshake 유무`]](#이전-논제-데이터-송수신의-3-way-handshake-유무)
    - [이전 논제 [`SEQ 번호에 담긴 값의 의미`]](#이전-논제-seq-번호에-담긴-값의-의미)
...
`슬라이딩 윈도우` 기법을 이용한다.
    ...
    수신측이 한번에 처리할 수 있는 데이터를 정해놓고

    그때그때 수신측의 데이터 처리 가능한 버퍼 사이즈를 윈도우 필드를 통해 송신측에게 알려줘서 데이터의 흐름을 제어하는 방식이다.

한번에 처리할 수 있는 데이터 즉 송신율은 다음과 같다.

    송신측에서 MSS와 윈도우가 동일하다면, RTT당 1개의 패킷

    다수의 MSS를 보낼만큼 윈도우가 크다면, RTT당 MSS/윈도우 개의 패킷

    수신측의 ACK 패킷 최소화도 해당된다.

    연속된 패킷이 수신된다면 TCP는 ACK 패킷을 하나로 합승한다.

`Stop-and-Wait` 기법과의 큰 차이는 
...
TCP는 `full-duplex`로 
    ...
    송·수신측은 각각 데이터를 담을 수 있는 수신 버퍼를 가지고 있고, 
    
    따라서 각각의 윈도우를 유지한다.

송신측은 이 윈도우 만큼의 데이터를 수신측의 ACK이 없어도 연속적으로 보낼 수 있다.
...
<details>
<summary>애플릿 시연의 과정은 이렇다.</summary>
<br/>

호스트 A는 호스트 B에게 4 kbytes의 파일을 전송한다.

각자의 수신 버퍼와 MSS는 2 kbytes이다.

1. 호스트 A는 패킷을 송신한다.
   
          SEQ     = 0
          데이터  = 2048 bytes (2 kbytes)

2. 호스트 B는 패킷을 수신한다. 2kbytes의 수신 버퍼가 가득 찼다.

          ACK     = 2048 (SEQ + MSS)
          WIN     = 0

3. 호스트 B의 수신 버퍼는 애플리케이션으로부터 비워져 가용하지만, 호스트 A는 이를 모른다. 아직 호스트 B가 가득찼다고 생각한다.
   
4. 호스트 A는 확인 패킷을 계속 전송한다.

          SEQ     = 2048
          데이터  = 1 byte

5. 호스트 B는 패킷을 수신하며 가용함을 알린다.

          ACK     = 2049
          WIN     = 2048

6. 호스트 A는 남은 데이터를 송신한다.

          SEQ     = 2048
          데이터  = 2048

7. 호스트 B는 파일을 전부 수신하였다.
...
`파이프라인 기법`에 의해 순서가 유지 되지 않게 수신된 상황은 

TCP 구현 개발자에게 맡겨진다고 기술되있고,

어떤 구현이 있는지 `파이프라인 오류 회복`에서 다뤄본다.

### 이전 논제 [`ACK 번호는 어떻게 계산되는가`]

`11주차 프로토콜 스택 Part2`에 등장한 논제이며 결론은 다음과 같다.

    ACK = SEQ + MSS

교재는 다음과 같이 의미를 더해주었다.
...
### 이전 논제 [`데이터 송·수신의 3-way handshake 유무`]

`16주차 웹서버` 주제때 등장한 논제이며 결론은 다음과 같다.

    접속 3-way handshake 외에도 

    데이터 송·수신 3-way handshake가 있을 수도 없을 수도 있다
...
그림을 통해 접속이 이루어진 뒤에 데이터 송·수신에 `3-way handshake`가 존재함을 확인할 수 있다.

<div align="center"><img width="60%" src="assets/telnet-data-transfer.jpg"/></div>
    
    송신측(클라이언트)이 입력한 문자는 원격 호스트에 송신되어
...
이러한 데이터 송·수신에 3-way handshake가 있는 경우에는 순서번호가 필수적이다.

### 이전 논제 [`SEQ 번호에 담긴 값의 의미`]

`12주차 LAN 어댑터`에서 다룬 논제였고 결론은 다음과 같다.

    접속에서 SEQ는            송·수신자의 미사용 값
    데이터 송·수신에서 SEQ는  송신할 조각 데이터의 맨 앞 위치

## 신뢰성 있는 데이터 전송의 원리

신뢰성 있는 데이터 전송은

    전송된 데이터가 손상되거나 손실되지 않고, 전송된 순서대로 애플리케이션에 전달된다.
...
**타임아웃 및 재전송**
    ...
    타임아웃 되는 상황은
      ...
      a. 송신자로부터 패킷이 손실되었을 때
      b. 수신자로부터 ACK 패킷이 손실되었을 때
      c. 패킷이 손실되지는 않았지만 시간이 지연되었을 때

    a, b의 상황에서는 송신자는 수신자로부터 중복된 확인응답을 받게된다.
...
<summary>ACK 패킷이 손실된다면</summary>
      ...
      2. 송신자가 p3의 ACK 패킷은 받았다면 수신자가 p2, p3를 오류 없이 받았음을 의미한다.
      - p2와 p3에 오류가 있었다면 p1에 대한 ACK 패킷을 받았을 것이다.
      3. 만약, 송신자가 p2, p3의 ACK 패킷도 받지 못했다면
      4. p2에 대한 타임아웃이 생긴다.
      5. 송신자는 p2, p3를 윈도우 사이즈 내에 한해서 동시에 재전송한다.
</details>
<br/>
...
[GBN / SR Applet](https://www2.tkn.tu-berlin.de/teaching/rn/animations/gbn_sr/) ━ *Johannes Kessler*

[Flow Control Applet](https://computerscience.unicam.it/marcantoni/reti/applet/FlowControl/flow.html) ━ *Pearson Education*
...
[수신버퍼의 정렬 필요성](https://dad-rock.tistory.com/269) ━ *Byeong Heon*