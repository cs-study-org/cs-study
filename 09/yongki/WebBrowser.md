...
정보가 포함되고, DNS 서버는 이에 대응하여 응답하는 항목이 테이블 또는 파일 형태로 등록되어 있다.

OS의 프로토콜 스택을 호출하여 **메시지 송신 실행을 의뢰**한다.
...
프로토콜 스택은 LAN 어댑터를 통해 메시지를 DNS 서버를 향해 송신한다.
...
2. 클라이언트측에서 소켓을 만드는데, 소켓이 생기면 돌아오는 디스크립터를 브라우저는 이를 메모리에 기록한다.

        디스크립터는
            데이터 송·수신 동작이 동시에 이뤄질 시, 
            브라우저 단에서 소켓을 식별하기 위해 사용한다.        

3. 이후 클라이언트측 소켓에서 서버측의 소켓에 연결한다.
