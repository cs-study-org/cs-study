...
기저 지식으로 this를 짚고 넘어가자.

this는
    
    this는 메서드의 호출 주체(객체)가 누군지를 확인하는 용도이다.

    this는 메서드가 어디서 정의되었는지에 상관없이 메서드를 호출한 주체에 따라 자유롭게 결정되기 때문이다.

    이 자유로운 점이 메서드를 하나만 만들어 여러 객체에서 재사용할 수 있다는 장점과
    이러한 유연함으로 객체 내의 프로퍼티로 묶는 캡슐화의 목적을 상실한다는 것이 단점이다.

화살표 함수를 쓰게된 이유는

    이러한 유연함을 없애기 위해서이다.

    화살표 함수 안에서 this는 화살표 함수가 선언된 시점의 주체를 가져오는데, 
    선언된 시점은 어떤 객체안의 프로퍼티인 점이 자명하므로

    이를 통해 캡슐화의 목적을 유지시킬 수 있다.
...
Javascript의 프로토타입이란
    ...
    Javascript의 모든 객체가 가장 전형적인 객체(Object)의 복제물이라는 뜻이다.
...
<img src="assets/memory-usage.png">

프로토타입 객체의 프로퍼티를 인스턴스에서 **접근**만 한다면, 
확인하여 참조만 하지만,

프로토타입 객체의 프로퍼티를 인스턴스에서 **연산(프로퍼티 추가나 오버라이딩)** 을 하려고 하는 경우,
