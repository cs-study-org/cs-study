...
하지만, TLB에서 해당되는 주소가 없을 경우 (TLB miss) 페이지테이블에서 주소이 존재하는지 찾는다.<br/> 
...
<br/>
CPU 내부의 있어 명령을 빠르게 수행 가능<br/>
-> 일반적으로 현재 계산을 수행중인 값을 저장하는데 사용<br/>

#### 레지스터 종류
- PC(Program Counter, 프로그램 카운터) : 다음에 수행될 명령어의 주소를 저장하는 레지스터
- IR(Instruction Register, 명령어 레지스터) : 현재 실행 중인 명령을 기억하는 레지스터
- MAR(Memory Address Register, 메모리 주소 레지스터) : 메모리에서 가져올 데이터의 주소를 저장하는 레지스터
- MBR(Memory Buffer Register, 메모 버퍼 레지스터) : 메모리에 기록되거나 메모리로부터 읽힐 데이터를 저장하는 레지스터
- I/O AR(Input/Output Address Register, 입출력 주소 레지스터) : 입출력 모듈의 주소를 가지고 있는 레지스터
- I/O BR(Input/Output Buffer Register, 입출력 버퍼 레지스터) : 입출력 모듈과 처리기 간의 교환하는 데이터를 저장하는 레지스터
- PSW(Program Status Word) : 여러 프로그램이 실행되면서 발생하는 상태를 저장하는 레지스터 ex) 인터럽트 유/무 체크, OS프로그램이 실행 되고 있을 때 체크 등
- AC(Accumulator, 누산기) : 연산 결과를 일시적으로 저장하는 레지스터
