...
지난 주차의 `서버에 접속한다`의 [접속](https://slides.com/kimyongki/deck-2ebcd7/embed)의 과정에서 TCP 오류 검출을 추가한 부분을 더해서 확인해보자.
...
1. 송신측은 TCP 헤더를 만들고, 송·수신처의 포트 번호를 지정한다.
    **송신측이 클라이언트라면 포트 번호는 지정되어 있지 않고, 미사용 값에서 적당히 할당한다.**
...
이에 앞서, 등장하는 단어의 역할을 짚어보자.
    ...
    ACK 번호:       수신한 조각 데이터의 맨 뒤 위치 + 1로, 그 다음 받을 조각 데이터의 맨 앞 위치를 통보
...
수신측의 말소 요청 패킷 이전에 송신측이 말소 되었다면,

수신측의 말소 요청 패킷이 

이전의 송신측의 소켓이 아닌, 새로 할당된 송신측의 소켓을 말소 해버릴 수 있다.

이를 방지하고자 송신측은 바로 말소 하지 않고, `TIME_WAIT`한다.
