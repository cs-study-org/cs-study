...
2. 애플리케이션의 종류나 만드는 방법에 따라 **분할해서 전송** / **한 번에 전송** 크게 2가지 방식으로 나눈다.(프로토콜 스택에서 제어 x)
...
**< 접속부터 송,수신까지 >**
1. 클라이언트 -> 서버(SYN : 1, 시퀀스 번호 초기값)
2. 서버 -> 클라이언트(ACK번호, SYN, 시퀀스 번호 초기값)
3. 클라이언트 -> 서버(ACK번호)
> 3-Way Handshaking이 위의 1,2,3이다.

4. 클라이언트 -> 서버(시퀀스 번호 , 데이터)
5. 서버 -> 클라이언트(ACK : 시퀀스 번호  + 데이터의 크기)
...

![connet, write 동작과정](asset/4.PNG)
...
### 데이터 보내기를 완료했을 때 연결을 끊는다.
- 데이터를 전부 송신했다면 데이터 송, 수신을 종료한다.
  - ex) 웹이라면 브라우저에서 웹 서버에 리퀘스트 메시지를 보네고, 서버가 이것을 응답하여 응답 메시지를 반송 완료하면 데이터를 전부 송신한 것이다.
  - 서버, 클라이언트 어느쪽에서든 먼저 연결 끊기 단계에 들어가도 좋게 프로토콜 스택은 구성되어있다.
- 서버측에서 연결 끊기를 했다고 가정하면
- 서버측의 애플리케이션이 먼저 Socket 라이브러리의 **close를 호출**한다.
- 서버측의 프로토콜 스택이 **TCP 헤더(FIN : 1)를 생성**
- IP담당 부분에 의뢰를 통해 클라이언트로 송신, 동시에 서버측의 소켓에 **연결 끊기로 상태 변경**
- 서버 -> 클라이언트로 TCP헤더(FIN)가 도착하면 클라이언트 소켓에 **연결끊기로 상태 변경.**
- **ACK 번호를 서버측으로 반송**하고 이것이 끝나면 애플리케이션이 데이터를 가지러 올 때 까지 대기한다.
- 서버측 애플리케이션이 read를 호출해 데이터를 가지러오면 데이터를 보내지 않고 서버에서 보낸 데이터를 전부 수신 완료 했다는 사실을 클라이언트측 어플리케이션(브라우저)에게 알린다.
- 웹의 동작은 서버가 응답을 반송하면 끝나도록 규칙이 정해져 있다. 서버에서 보낸 데이터를 전부 수신 완료하면 클라이언트도 종료(클라이언트측 어플리케이션이 close를 호출하여 데이터 송,수신 동작 끝냄)
- 클라이언트측의 프로토콜 스택이 **TCP 헤더(FIN : 1)를 생성 후**, IP담당 부분에 의뢰를 통해 서버로 송신
- **서버에서 ACK번호가 돌아오면 서버와의 대화가 끝난다.**

![close 동작과정](asset/7.PNG)

### 소켓을 말소한다.
- 바로 소켓을 말소하지 않고 잠시 기다린 후 소켓을 말소한다.
- 오동작을 막기 위해 일반적으로 **몇 분 정도 기다리고 나서 소켓을 말소**한다.
  - 소켓이 새로 생긴 다른 소켓으로 전송되는 것을 막는다.
  - 재전송된 패킷이 네트워크에 존재할 가능성이 있기 때문에
...
## 참고문헌
[네트워크 원리 참고문헌](https://lelana.tistory.com/140)
