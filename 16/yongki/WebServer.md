...
하나의 프로그램을 여러 클라이언트가 사용하기 위해서 이전 지식을 재정립할 필요가 있다.

포트 번호는 
    
    클라이언트 측에서는 소켓 한개를 담당하는 것이며,

    서버 측에서는 PC의 프로그램을 식별하는데 사용하며
...
이를 통해 학부 수업에서 사용해본 소켓 처리 방식을 떠올릴 수 있었다.
...
<center><img width="60%" src="assets/thread-pool.drawio.svg"/></center>

    스레드 개수에 제한을 둬서 한번에 만들어놓고, (스레드풀)
