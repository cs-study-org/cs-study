...
해당 **택배의 최종 목적지 정보(주소)** 가 담겨 있기 때문이다. 그리고 중간중간 거치는 **택배 물류 Hub**의 경우에는 네트워크를 이루는 **라우터**라고 할 수 있다. 왜냐하면 각 Hub에서는 **가장 빠른 경로로 택배를 운송하기 위해 적절한 트럭에 상차시키는 행위(간선 상차)**를 하기 때문이다. 이 행위가 라우팅이다.
