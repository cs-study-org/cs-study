...
홍길동 25 영어
```
부모 클래스가 같은 이름의 메서드를 갖고 있더라도, 자식도 같은 메서드를 갖고 있는 경우 본인의 메서드를 우선적으로 수행함. 이를 오버라딩이라고 한다.
...
#### 레퍼런스 카운트의 약점(순환 참조)
- 순환 참조란 간단학 컨테이너 객체가 자기 자신을 참조하는 것을 의미한다. 자기 자신이 참조될 때 프로그래머는 할당된 객체를 추적하기 어려워지고, 이때 메모리 누수가 발생 할 수 있다.
...
#### 세대관리
파이썬은 객체 관리를 위한 영역을 3가지로 나누고 이역역을 세대(generation)라고함
