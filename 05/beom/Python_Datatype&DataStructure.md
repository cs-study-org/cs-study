...
> 단점 : 
> 빌드 되어 있는 컴파일 언어 프로그램보다 실행시간이 느린다.
> 코드를 열면 다 보이기 때문에 보안에 취약하다.
...
한방향 연결리스트의 단점 : 한쪽으로만 연결이 되어 있기 때문에 테일 노드에 대한 조작을 하고 싶을 때 직전의 노드(previous node)를 알고 있어야한다. 그래서 O(n)의 연산 시간을 사용한다.

위 문제를 해결하기 위해 양방향 연결리스트 등장
...
- 파이썬에서는 해쉬를 별도로 구현할 필요 없음. 왜 Why? 딕셔너리 타입을 사용하면 되기 때문에
