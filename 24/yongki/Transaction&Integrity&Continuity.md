...
트랜잭션은 Script 이다.
    ...
    각 단위는 단일 SQL문을 의미할 수도 의미하지 않을 수도 있다.
...
데이터 부정합을 방지하고자
    ...
    복구하는 작업을 애플리케이션에서 사전에 방지하고자 테스트로 대비하는 것도 가능하지만
...
정답은
    ...
    5-6번은 반드시 하나의 트랜잭션으로 처리해야하며

    7번은 조회이므로 트랜잭션이 필요없고,

    8-9번은 별도의 트랜잭션으로 처리하지만, 네트워크 통신은 트랜잭션에서 배제하는 것이 좋다.

    통신되지 않는 상황이 발생하면 DBMS 까지 영향을 받는다.
...
> 🤔 소스 서버는 바이너리 로그에 입력받은 SQL문을 데이터에 실행시킬 때, 
> InnoDB 테이블에도 반영 시키는것인가
> 만약 이것이 T1, T2가 이루어졌다고 하였을 때
> 
> 차후 레플리카 서버에서 장애가 발생하여 T2는 릴레이 로그에 들어오지 않을 시
> T2에 해당한 InnoDB의 트랜잭션을 레플리카로 다시 넘겨주는 것인가


> 🤔 소스 서버는 트랜잭션이 커밋될 때마다 그 갱신 SQL 문을 바이너리 로그에 기록한다?
> 바이너리 로그에 입력받은 뒤에 차후에 데이터에 커밋하는것 아닌가
...
MySQL5.5(InnoDB)에는 격리 수준을 MVCC(Multi Version Concurrency Control)로 구현하였다.
    ...
    locking을 사용하지 않아, 일관된 작업이 가능하게 한다.

    하나의 레코드에 대해 2개의 버전이 디스크와 메모리(언두 로그)에 유지되고, 격리 수준에 따라 달라진다.
...
2. Toto는 디스크에 즉시 기록되고, Lara는 언두 로그로 백업된다.
