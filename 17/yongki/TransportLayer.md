...
<div align="center"><img width="60%" src="assets/layer-example.drawio.svg"/></div>

교재에 일반화한 예시가 있었다.

의문점을 가진 부분이 다중화에 있었는데,

L4 프로토콜은 L7의 소켓으로부터 데이터를 받아 

각 데이터(편지)마다 세그먼트(봉투)로 만든뒤, 이들을 모아 L3 프로토콜에 전달함으로

세그먼트들이 하나의 패킷이 되어 보내진다고 오해하였다.

여기서, 모아지는 부분은 세그먼트가 만들어지기 이전이다.

상위 계층인 L7의 데이터를 모아 세그먼트 하나를 만드는 행위인 것이었다.

즉, 교재에 서술한 예시를 재서술하자면

P1, P2의 각 편지의 일부분이 하나의 봉투로 들어가야 한다.
...
[다중화의 모아지는 대상](https://donghoson.tistory.com/37) ━ *끄적끄적 동커벨*